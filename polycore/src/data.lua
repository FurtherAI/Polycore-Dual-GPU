...
--[[
print("beginning")
print(read_cmd("nvidia-smi --format=csv,noheader,nounits --query-gpu=memory.total"))
print("beginning")
print(read_cmd("nvidia-smi --format=csv,noheader,nounits --query-gpu=memory.total,memory.used,utilization.gpu"))]]
...
local nvidia_loader = EagerLoader(function(vars)
    ...
    local out = output:gmatch("([^\n]+)")  -- split at newline, each one in it should be a single gpu's reading
    local out_both = {}  -- make array/table
    local ct = 1
    for result in out do
        out_both[ct] = (", " .. result):gmatch(", ([^,]+)")  -- insert iterator over values in each
        ct = ct + 1
    end
    output = ""
    for gpu1 in out_both[1] do  -- concat gpu1 and gpu2 values with newline, then return iterator over the gpu1/2 pairs
        output = output .. ", " .. gpu1 .. "\n" .. out_both[2]()
    end
    return output:gmatch(", ([^,]+)")
...
function get_from_gpu_(idx, reading)
    local result = nvidia_loader:get(reading)
    local ct = 1
    for gpu_result in string.gmatch(result, "[^\r\n]+") do
        if (ct == idx) then return gpu_result else ct = ct + 1 end
    end
end
...
-- @treturn number
function data.gpu_percentage(idx)
    return tonumber(get_from_gpu_(idx, "utilization.gpu"))
end

--- Get current GPU frequency.
-- @treturn number in MHz
function data.gpu_frequency(idx)
    return tonumber(get_from_gpu_(idx, "clocks.current.graphics"))
end

--- Get current GPU temperature.
-- Relies on nvidia-smi to be installed.
-- @treturn number temperature in °C
function data.gpu_temperature(idx)
    return tonumber(get_from_gpu_(idx, "temperature.gpu"))
end

--- Get current VRAM usage.
-- Relies on nvidia-smi to be installed.
-- @treturn number,number used, total in MiB
function data.gpu_memory(idx)
    return tonumber(get_from_gpu_(idx, "memory.used")),
           tonumber(get_from_gpu_(idx, "memory.total"))
end

--- Get current GPU power draw.
-- Relies on nvidia-smi to be installed.
-- @treturn number power draw in W
function data.gpu_power_draw(idx)
    return tonumber(get_from_gpu_(idx, "power.draw"))
end

--- Get current GPU power draw.
-- Relies on nvidia-smi to be installed.
-- @treturn number power draw in W
function data.gpu_power_limit(idx)
    return tonumber(get_from_gpu_(idx, "power.limit"))
end

--- Get list of GPU processes with individual VRAM usage in MiB.
-- Relies on nvidia-smi to be installed.
-- @treturn {{string,number},...} list of {name, mem} value pairs.
function data.gpu_top(idx)
    local output = read_cmd("nvidia-smi --id=" .. idx .. " -q -d PIDS")
    local processes = {}
...
data.device_temperatures = util.memoize(5, function(device)
    local temp_inputs = read_cmd("ls -1"
        ...
        .. " /sys/class/hwmon/hwmon*/temp1_input"  -- fix, these files exist, but not in those directories
    )
    local temps = {}
