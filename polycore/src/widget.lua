...
--- no options
function Gpu:init(idx)
    self.idx = idx
    self._usebar = Bar{ticks={.25, .5, .75}, unit="%"}

    local _, mem_total = data.gpu_memory(self.idx)
    self._membar = MemoryBar{total=mem_total / 1024}
    self._membar.update = function()
        self._membar:set_used(data.gpu_memory(self.idx) / 1024)
    end
    Rows.init(self, {self._usebar, Filler{height=6}, self._membar})
end

function Gpu:update()
    self._usebar:set_fill(data.gpu_percentage(self.idx) / 100)

    local color = {w.temperature_color(data.gpu_temperature(self.idx), 30, 80)}
...
function GpuTop:update()
    self._processes = data.gpu_top(self.idx)
end
